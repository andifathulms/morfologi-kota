 ...
 */
export function ModeSwatch({ mode }: { readonly mode: Mode }) {
...
export function ModeKey({ locale, className }: { readonly locale: Locale; readonly className?: string }) {
  const rows = [
    { mode: 'drive' as const, term: d('drive', locale), gloss: d('keyDrive', locale) },
    { mode: 'walk' as const, term: d('walk', locale), gloss: d('keyWalk', locale) },
  ]
        ...
        {rows.map((row) => (
          <div key={row.mode} className="contents">
            <dt className="flex items-center gap-2 font-sans text-base font-semibold">
              ...
              <span style={{ color: row.mode === 'drive' ? 'var(--drive)' : 'var(--walk)' }}>
                {row.term}
              </span>
            </dt>
